...
const ArticlesList = () => {
  return (
    <main className="py-16">
      <SEO title="Articles – Blog" description="All articles and blog posts" />
      <section className="container mx-auto px-6">
        <header className="mb-10 text-center">
          <h1 className="text-4xl font-bold">Articles – Blog</h1>
          <p className="text-muted-foreground mt-2">Blog posts about digital marketing, automation and analytics.</p>
        </header>
          ...
          {articles.map((a) => (
            <article key={a.id} className="bg-card border border-border rounded-xl overflow-hidden shadow-card">
              <div className="aspect-[4/3] overflow-hidden">
                <img src={a.image} alt={`Article: ${a.title}`} className="w-full h-full object-cover" loading="lazy" />
              </div>
              <div className="p-6 space-y-3">
                ...
                <Button asChild variant="outline" className="w-full">
                  <Link href={`/articles/${a.id}`} aria-label={`Read more: ${a.title}`}>
